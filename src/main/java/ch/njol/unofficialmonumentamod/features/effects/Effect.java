...
public class Effect {
    ...
    public Text toText(float tickDelta, boolean rightAligned) {
        Text timeText = Text.literal((rightAligned ? " " : "") + getTimeRemainingAsString(tickDelta) + (rightAligned ? "" : " "));
        ...
        Text effectText = Text.literal(effectString).setStyle(effectStyle);
        ...
        if (textRenderer.getWidth(effectText) > maxEffectWidth) {
            ...
            effectText = Text.literal(effectString.substring(0, trimmedLength) + "...").setStyle(effectStyle);
        }
        return rightAligned ? Text.literal("").append(effectText).append(timeText)
                : Text.literal("").append(timeText).append(effectText);
    }
